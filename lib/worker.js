...
var processScript = function(fn, callback) {
    ...
    async.series([
        ...
        function(callback) {
            ...
            tmp.dir(function _tempDirCreated(err, tmpDir) {
                ...
                async.series([
                    ...
                    function(callback) {
                        reader.splitScript(fn, dir, function(err, msg) {
                            if (err) return callback(err);
                            nPages = msg;
                            log.info(nPages + ' jpegs created');
                            callback();
                        });
                    },
                    function(callback){
                        connect.findBooklet(script, function(err, booklet){
                            if(err) return callback(err);
                            if(booklet.pages!=nPages) return callback(new Error('Wrong number of pages: ' + script));
                            log.info('Correct number of pages scanned');
                            callback();
                        });
                    },
        ...
        function(callback) {
            ...
            connect.updateScript(task, qrcode, function(err) {
                if(err) return callback(err);
                callback();
            });
        },

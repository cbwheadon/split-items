...
var insertResponse = function (script, outFile, item, callback) {
    ...
    if (err) {
        ...
        db.collection('responses',function(err,collection){
            collection.insert({filePath: outFile, booklet: script.booklet, task:script.task, owners:script.owners, item:item._id, timesToMark:item.timesToMark, markingComplete: false, script: script._id, createdAt:  moment().format()}, function(err) {
                db.close();
                callback(err);
            });
        });

...
var findItems = function (script,dir,callback) {
    // Gets the players from the database and returns as array
    var MongoClient = require('mongodb').MongoClient, format = require('util').format;
    MongoClient.connect(config.get('mongoUri'), function (err, db) {
    if (err) {
...
var insertResponse = function (script, outFile, item, callback) {
    ...
    if (err) {
        ...
        db.collection('responses',function(err,collection){
            collection.insert({filePath: outFile, booklet: script.booklet, task:script.task, owner:script.owner, item:item._id, script: script._id, rand:Math.random()}, function(err, outFile) {
